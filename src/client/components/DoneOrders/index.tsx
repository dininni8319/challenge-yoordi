...
const DoneOrders = ({ doneOrders , show }:{ doneOrders:IOrder[], show: boolean } ):JSX.Element => {
   ...
   return (
      <div className={AppCss.container}>
         <div>
               {
                  doneOrders?.sort((a:IOrder, b: IOrder) => a.date - b.date)
               .reverse()
               .map((item: IOrder) => {
                  return ( 
                        <ul key={`order${item._id}`} className={AppCss.card} style={{border:`2px solid ${show ? '#188d20': '#f94144'} `}}>
                           ...
                           <li className={AppCss.cardTitle}>
                              <span>Amount ${item?.card && getTotalPrice(item?.card)}</span>
                           </li>
                     { orderDetail &&
                     currentId === item._id 
